...
export const selectThresholds = createSelector([(state: RootState) => state.coreData.auxiliaryData], (auxiliaryData) => {
  ...
  if (!thresholds) {
    console.warn("DEBUG: selectThresholds: No thresholds data available");
    return [];
  }
...
export const selectStateThresholds = createSelector(
  ...
  (auxiliaryData, stateNum) => {
    ...
    if (!thresholds || !thresholds[stateNum]) {
      console.warn(`DEBUG: selectStateThresholds: No thresholds for state ${stateNum}`);
      return null;
    }
...
export const selectLocationData = createSelector([(state: RootState) => state.coreData.auxiliaryData], (auxiliaryData) => {
  if (!auxiliaryData?.locations) {
    console.warn("DEBUG: selectLocationData: No location data available");
    return [];
  }
...
export const selectNowcastTrends = createSelector([(state: RootState) => state.coreData.mainData], (mainData) => {
  if (!mainData?.nowcastTrends) {
    console.warn("DEBUG: selectNowcastTrends: No nowcast trends available");
    return [];
  }

  // Transform nested structure to array format expected by components
  const nowcastData = Object.entries(mainData.nowcastTrends).map(([modelName, modelData]) => ({
    modelName,
...
export const selectGroundTruthInRange = createSelector(
  ...
  (timeSeriesData, seasons, { startDate, endDate }, stateNum) => {
    if (!timeSeriesData || !seasons) {
      console.warn("DEBUG: selectGroundTruthInRange: Missing required data");
      return [];
    }
    ...
    if (relevantSeasons.length === 0) {
      console.warn("DEBUG: selectGroundTruthInRange: No relevant seasons found");
      return [];
    }
    ...
    for (const seasonId of relevantSeasons) {
      ...
      // Get any model's data (ground truth duplicated across all models)
      const firstModel = Object.keys(seasonData).find(
        (key) => key !== "firstPredRefDate" && key !== "lastPredRefDate" && key !== "lastPredTargetDate"
      );
...
export const selectPredictionsForModelAndWeek = createSelector(
  ...
  (timeSeriesData, seasons, modelName, stateNum, referenceDate) => {
    ...
    if (!seasonId) {
      console.warn("DEBUG: selectPredictionsForModelAndWeek: No season found for date", dateISO);
      return null;
    }

    const modelData = timeSeriesData[seasonId]?.[modelName];
    if (!modelData) {
      console.warn("DEBUG: selectPredictionsForModelAndWeek: No model data", { seasonId, modelName });
      return null;
    }
    ...
    console.warn("DEBUG: selectPredictionsForModelAndWeek: No predictions found");
    return null;
  }
);
