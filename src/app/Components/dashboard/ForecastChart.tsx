...
const ForecastChart: React.FC = () => {
  ...
  function processPredictionData(
    ...
    if (displayMode === "byDate") {
      ...
      Object.entries(modelData).forEach(([modelName, predictionData]) => {
        let filteredByReferenceDate = predictionData.filter(
          (d) =>
            d.referenceDate.getFullYear() === selectedWeek.getFullYear() &&
            d.referenceDate.getMonth() === selectedWeek.getMonth() &&
            d.referenceDate.getDate() === selectedWeek.getDate(),
        );
        ...
        let filteredByTargetEndDate = filteredByReferenceDate.filter((d) => {
          ...
          return (
            d.targetEndDate >= d.referenceDate && d.targetEndDate <= targetWeek
          );
        });
  ...
  function createScalesAndAxes(
    ...
    const xAxis = d3
      ...
      .tickFormat((d: Date) => {
        ...
        const isNearYearChange =
          (d.getMonth() === 11 && d.getDate() >= 24) ||
          (d.getMonth() === 0 && d.getDate() <= 14);
    ...
    if (predictions && Object.keys(predictions).length > 0) {
      ...
      maxPredictionValue =
        predictionValues.length > 0 ? d3.max(predictionValues) : 0;
    }
    ...
    if (yAxisScale === "linear") {
      ...
      const minPositiveValue =
        d3.min(
          ground.filter((d) => d.admissions > 0),
          (d) => d.admissions,
        ) || 1;
      yScale = d3
        .scaleSymlog()
  ...
  function updateCornerTooltip(
    ...
    const admissionsText = cornerTooltip
      ...
      .text(
        `Admissions: ${data.admissions !== null && data.admissions !== -1 ? data.admissions : "N/A"}`,
      );

    console.log("AdmissionText Node: ", admissionsText.node());
    maxWidth = Math.max(
      maxWidth,
      admissionsText.node().getComputedTextLength(),
    );
    currentY += lineHeight;
    ...
    if (currentPredictions) {
      Object.entries(currentPredictions).forEach(
        ([modelName, modelData]: [string, any]) => {
          ...
          maxWidth = Math.max(
            maxWidth,
            modelText.node().getComputedTextLength() + 15,
          );
          currentY += lineHeight;
          ...
          maxWidth = Math.max(
            maxWidth,
            medianText.node().getComputedTextLength() + 15,
          );
          currentY += lineHeight;
          ...
          confidenceInterval.forEach((interval) => {
            ...
            const ciText = cornerTooltip
              ...
              .attr("fill", "white")
              .text(
                `${interval}% CI: [${modelData[CILowKey].toFixed(2)}, ${modelData[CIHighKey].toFixed(2)}]`,
              );

            maxWidth = Math.max(
              maxWidth,
              ciText.node().getComputedTextLength() + 15,
            );
            currentY += lineHeight;
          });
  ...
  function findPredictionsForDate(predictionData: any, date: Date) {
    ...
    Object.entries(predictionData).forEach(
      ([modelName, modelPredictions]: [string, any]) => {
        const prediction = modelPredictions[0].data.find(
          (p: any) => new Date(p.targetEndDate).getTime() === date.getTime(),
        );
        if (prediction) {
    ...
    Object.values(predictionData).forEach((modelPredictions: any) => {
      ...
      modelPredictions[0].data.forEach((prediction: any) => {
        const existingPoint = combinedData.find(
          (d) =>
            d.date.getTime() === new Date(prediction.targetEndDate).getTime(),
        );
        if (!existingPoint) {
          combinedData.push({
  ...
  function renderChartComponents(
    ...
    const cornerTooltip = createCornerTooltip(
      svg,
      marginLeft,
      marginTop,
      chartWidth,
  ...
  useEffect(() => {
    ...
    if (svgRef.current && groundTruthData.length > 0) {
      ...
      if (allPlaceholders) {
        ...
        renderMessage(
          svg,
          "Not enough data loaded, please extend date range",
          chartWidth,
          chartHeight,
          marginLeft,
          marginTop,
        ...
        updateVerticalIndicator(
          userSelectedWeek || filteredGroundTruthData[0].date,
          xScale,
          marginLeft,
          chartWidth,
          verticalIndicatorGroup,
