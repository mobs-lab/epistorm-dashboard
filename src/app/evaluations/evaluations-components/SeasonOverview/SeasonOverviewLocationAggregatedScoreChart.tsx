...
} from '@/store/selector/evaluationSelectors'
...
> = ({ type }) => {
    ...
    const processedData = useMemo(() => {
        if (shouldUseJsonData && seasonOverviewData) {
            ...
            const relevantEvaluations = evaluationsScoreData.filter(
                (data) => data.scoreMetric === scoreMetric
            )
            const horizonSet = new Set(evaluationSeasonOverviewHorizon)
            const results: IQRData[] = []

            for (const modelName of modelNames.filter((model) =>
                evaluationSeasonOverviewSelectedModels.includes(model)
            )) {
                const modelData = relevantEvaluations.find(
                    (data) => data.modelName === modelName
                )
                ...
                for (const entry of modelData.scoreData) {
                    ...
                    if (
                        referenceDate < selectedPeriod.startDate ||
            ...
            console.debug(
                'Using CSV fallback data for aggregated chart:',
                results
            )
            return results
        }
    ...
    const generateEvenlySpacedTicks = (
        ...
        if (isWis) {
            ...
            if (maxValue <= 1) {
                ...
                // For larger range
                ticks = [0, 0.5, 1, 2, 3, 4, 5, 6, 7, 8].filter(
                    (v) => v <= maxValue
                )
            }
            ...
            const uniqueValues = [...new Set([0, ...dataValues])].sort(
                (a, b) => a - b
            )
            return uniqueValues
        }
    ...
    const renderChart = () => {
        ...
        const scaleType =
            type === 'wis' ? wisChartScaleType : mapeChartScaleType
