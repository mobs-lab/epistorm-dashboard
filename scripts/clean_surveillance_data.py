...
# NOTE: This function might needs to be changed when this file moves.
...
def clean_surveillance_data():
    """
    Clean newly-retrieved ground truth data (from CDC repo, git submodule)
    """
    ...
    current_data_path = os.path.join(
        project_root,
        "data_processing_dir/raw/ground-truth/compare/target-hospital-admissions.csv",
    )

    output_data_path = os.path.join(project_root, "data_processing_dir/raw/ground-truth/target-hospital-admissions.csv")

    historical_data_dir = os.path.join(project_root, "data_processing_dir/raw/ground-truth/historical-data")
    ...
    if os.path.exists(current_data_path):
        ...
        print("Current surveillance data processed:")
    ...
    if os.path.exists(historical_data_dir):
        for filename in os.listdir(historical_data_dir):
            if filename.endswith(".csv"):
                ...
                print("Historical file processed:")
                print(f"  - Original rows: {original_count}")
                print(f"  - Rows removed: {original_count - len(df)}")
                print(f"  - Remaining rows: {len(df)}")
