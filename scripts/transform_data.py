...
def data_transformation(team_name):
    ...
    predictions_target_path = os.path.join(project_root, "public", "data", "processed", team_name, "predictions.csv")

    trends_target_path = os.path.join(project_root, "public", "data", "processed", team_name, "nowcast_trends.csv")
    ...
    team_data = pd.concat(
        (
            pd.read_csv(
                f,
                usecols=["reference_date", "target", "target_end_date", "location", "output_type_id", "value"],
                dtype={"location": str, "output_type_id": str},
                parse_dates=True,
            )
            for f in glob.glob(team_data_source_pattern)
        ),
        ignore_index=True,
    ...
    if team_name in ("MOBS-GLEAM_FLUH", "MIGHTE-Nsemble", "CEPH-Rtrend_fluH", "FluSight-ensemble", "NU_UCSD-GLEAM_AI_FLUH", "MIGHTE-Joint"):
        ...
        trends = (
            trends.groupby(["reference_date", "location", "output_type_id"], as_index=False)
            .sum()
            .pivot_table(values="value", index=["location", "reference_date"], columns=["output_type_id"])
            .reset_index()
        )
